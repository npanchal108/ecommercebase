...
export class rmaComponent implements OnInit {
  ...
  constructor(private formBuilder: UntypedFormBuilder, el: ElementRef, private renderer: Renderer2, private seoService: SEOService, private registerService: RegistrationService, private routingState: RoutingState, private checkoutService: CheckoutService, private cartService: CartService, private toastr: ToastrService, private dataService: DataService, private router: Router, private route: ActivatedRoute) {
    ...
    this.seoService.setPageTitle('RMA - ' + geturl);
    this.seoService.setkeywords('RMA - ' + geturl);
    this.seoService.setdescription('RMA - ' + geturl);
    ...
    if (this.rfqid != undefined && this.rfqid != null && this.rfqid != '') {
      this.dataService.GetUserwishlist(Common.getWithExpiry("CustID"), 3).subscribe((res: any) => {
        this.wishlist = res;
      ...
      this.dataService.Addwishlistheader(null, name1, Common.getWithExpiry("CustID"), null, 3).subscribe((res: any) => {
        var results = res;
        if (results > 0) {
          this.getwishlist();
  ...
  getrmalable() {
    this.RMALable = Common.getWithExpiry("RMALable");
    if (this.RMALable == null || this.RMALable == undefined) {
      this.dataService.GetRMALable().subscribe((data: any) => {
        this.RMALable = data;
        Common.setWithExpiry("RMALable", this.RMALable);
      });
    }
  }
  ...
  SaveOrder() {
    ...
    Common.setWithExpiry("rmamodel", JSON.stringify(rfqmodel));
  }
  selectsavedorder() {
    try {
      if (Common.getWithExpiry("rmamodel") != undefined) {
        var getsaved = JSON.parse(Common.getWithExpiry("rmamodel"))
      }
  ...
  AddNewItem1() {
    this.dataService.getProductDetailName(this.newitem, this.warehouse, Common.getWithExpiry("CustID")).subscribe((res: any) => {
      var item1 = res;
  ...
  Addwishlistheader(WishlistName) {
    if (WishlistName == undefined || WishlistName == undefined || WishlistName == null || WishlistName == '') {
      ...
      this.dataService.Addwishlistheader(this.currentwishlist.WishlistID, this.currentwishlist.WishlistName, Common.getWithExpiry("CustID"), null, 3).subscribe((res: any) => {
        var results = res;
        if (results == 0) {
          this.toastr.error("Error Occured Please Try Again", 'Message!');
        }
  ...
  finalizeOrder() {
    if (this.ship_to == undefined || this.ship_to == null || this.ship_to == '') {
    ...
    if (this.ship_to == '-1' && (this.shipping.PostalCode == undefined || this.shipping.PostalCode == null || this.shipping.PostalCode == '')) {
      ...
      for (var i = 0; i < this.currentproductlist.length; i++) {
        ...
        if (this.currentproductlist[i].keycode == undefined || this.currentproductlist[i].keycode == null || this.currentproductlist[i].keycode == '') {
          this.toastr.error("Please Key Code");
          const element = this.renderer.selectRootElement("#keycode"+i);
          element.focus();
          return;
        }
        if (this.currentproductlist[i].expdate == undefined || this.currentproductlist[i].expdate == null || this.currentproductlist[i].expdate == '') {
          this.toastr.error("Please Expiration Date");
          const element = this.renderer.selectRootElement("#expdate"+i);
          element.focus();
          return;
        }
        if (this.currentproductlist[i].lotno == undefined || this.currentproductlist[i].lotno == null || this.currentproductlist[i].lotno == '') {
          this.toastr.error("Please Lot Number");
          const element = this.renderer.selectRootElement("#lotno"+i);
          element.focus();
          return;
        }
        ...
        lines.push((this.TextUpperCase == '1' ?
          this.ConvertKeysToUpperCase({
            ...
            "notes": "KeyCode: "+this.currentproductlist[i].keycode+"; ExpirationDate: "+this.currentproductlist[i].expdate+ "; LotNumber: "+this.currentproductlist[i].lotno+";",
          }) :
          {
            "order_ext": ordExt, //product.descr
            "customer": Common.getWithExpiry("CustID"),
      ...
      this.checkoutService.FinalizeOrderRMA(model).subscribe((res: any) => {
        this.orderno = res;
