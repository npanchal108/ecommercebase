...
export class PendingOrderComponent implements OnInit {
  ...
  constructor(private seoService: SEOService, private dataService: DataService, private router: Router, private orderService: OrderManagementService) {
    ...
    this.seoService.setPageTitle('Pending Orders - ' + geturl);
    this.seoService.setkeywords('Pending Orders - ' + geturl);
    this.seoService.setdescription('Pending Orders - ' + geturl);
  ...
  getProcessingOrders(pageno) {
    ...
    this.orderService.getProcessingOrders(Common.getWithExpiry("CustID"), usrid, pageno, this.sorttype).subscribe((res: any) => {
      var result = res;
      ...
      for (var i = 0; i < result.length; i++) {
        this.processingOrderList.push({
          ...
          "order_ext": result[i].order_ext,
          "cu_po": result[i].cu_po,
          "tot_code_amt": result[i].tot_code_amt[0],
          "customer": result[i].customer,
          "pay_amount": result[i].pay_amount,
  ...
  getPendingOrders(pageno) {
    ...
    this.orderService.getPendingOrders(Common.getWithExpiry("CustID"), usrid, pageno, this.sorttype).subscribe((res: any) => {
      var result = res;
      this.orderList = [];
      for (var i = 0; i < result.length; i++) {
  ...
  getPendingOrdersCount() {
    ...
    this.orderService.getPendingOrdersCount(Common.getWithExpiry("CustID"), usrid).subscribe((res: any) => {
      var getdata = res;
