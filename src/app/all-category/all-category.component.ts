...
export class AllCategoryComponent implements OnInit {
  ...
  constructor(private seoService: SEOService,private loadingService: LoadingService,private toastr: ToastrService, private sanitizer: DomSanitizer, private cartService: CartService, private dataService: DataService, private menuService: MenuService, private router: Router, private route: ActivatedRoute) {
    try {
  ...
  ondivclick(category) {
    if (category.parent != 0) {
      this.router.navigate(['/categories', category.tree_node.toLowerCase()]);
    }
    else {
      this.router.navigate(['/category', category.tree_node.toLowerCase()]);
    }
  }
    ...
    if (this.Allconfigurationlist == null || this.Allconfigurationlist == undefined || this.Allconfigurationlist == '') {
      this.dataService.GetAllConfiguration().subscribe((data: any) => {
        ...
        for (var i = 0; i < this.Allconfigurationlist.length; i++) {
          if (this.Allconfigurationlist[i].ConfigKey == "CategoryLableShow") {
            this.showcatlable = this.Allconfigurationlist[i].ConfigValue;
          }
          if (this.Allconfigurationlist[i].ConfigKey == "ProductView") {
            this.isListView = this.Allconfigurationlist[i].ConfigValue;

          }
          if (this.Allconfigurationlist[i].ConfigKey == "DisplayCounts") {
            this.DisplayCounts = this.Allconfigurationlist[i].ConfigValue;
          }
        }
      })
    }
  ...
  GetconfigurationforDisplayCounts() {
    this.DisplayCounts = this.dataService.Getconfigbykey("DisplayCounts");
  ...
  GetConfigtoCategoryLableShow() {
    this.showcatlable = this.dataService.Getconfigbykey("CategoryLableShow");
    if (this.showcatlable == null || this.showcatlable == undefined || this.showcatlable == '') {
      this.showcatlable = Common.getWithExpiry("showcatlable");
    }
    if (this.showcatlable == null || this.showcatlable == undefined || this.showcatlable == '') {
      this.dataService.GetConfigtoCategoryLableShow().subscribe((res: any) => {
        this.showcatlable = res;
        Common.setWithExpiry("showcatlable", this.showcatlable);
      });
    }
  }
  ...
  getProductView() {
    var ProductView = this.dataService.Getconfigbykey("ProductView");
    if (ProductView == null || ProductView == undefined || ProductView == '') {
      ProductView = Common.getWithExpiry("ProductView");
    }
    if (ProductView == null || ProductView == undefined || ProductView == '') {
  ...
  GetFirstpageload() {
    ...
    this.route.params.subscribe(params => {
      ...
      this.treeNode = this.route.snapshot.paramMap.get('name');
      this.categoryList = [];

      if (this.treeNode == "" || this.treeNode == null || this.treeNode == undefined) {
        ...
        this.getAllCategory();
        this.Getcategoriespath1(this.treeNode);
        ...
        Common.setWithExpiry('selectedprod_line', '');
        Common.setWithExpiry('selecctedsatags', '');
        Common.setWithExpiry('selectedtreenode', this.treeNode);
        this.filterMenuByTreeNode(this.treeNode);

      }
      if (this.iskyraden && this.treeNode!=undefined && this.treeNode!=null && this.treeNode!='') {
  ...
  getsalablesfilters() {
    ...
    this.safilterslist = [];
    if (Common.getWithExpiry('safilterslistparent' +this.treeNode + getsatags + words) != undefined) {
      try {
        this.safilterslistparent = JSON.parse(Common.getWithExpiry('safilterslistparent' +this.treeNode + getsatags + words));
      } catch (ed) { }
    }
    if (this.safilterslistparent == null || this.safilterslistparent == undefined || this.safilterslistparent.length == 0) {
      ...
      this.menuService.getsafilters(null, null, this.treeNode, JSON.stringify(this.selecctedsatags), words, warehouse, Common.getWithExpiry('CustID'), Common.getWithExpiry("company_sy")).subscribe((res: any) => {
        ...
        Common.setWithExpiry('safilterslistparent' + this.treeNode + getsatags, JSON.stringify(this.safilterslistparent));
      })
    }
    else {
...
if (((this.category != undefined && this.category != null && this.category != '') || (getsatags != undefined && getsatags != null && getsatags != '' && getsatags.length > 1))) {
      //this.sendMessage('start');
  ...
  setmetaandtitle() {
    ...
    this.dataService.GettreenodebyNode(this.treeNode).subscribe((data: any) => {
      this.treedetails = data;
      console.log('this.treedetails',this.treedetails);
      if (this.treedetails != undefined && this.treedetails != null) {
        //var geturl = Common.getWithExpiry("cpname");      
        this.seoService.createLinkForCanonicalURLforproduct(encodeURIComponent(this.treedetails.tree_node) + '/' + this.treedetails.name.replace(/[^A-Z0-9]/ig, "-").replace(/---/g, '-').replace(/--/g, '-'));
        if (this.treedetails.meta != undefined && this.treedetails.meta != null && this.treedetails.meta != '') {
          this.seoService.setkeywords(this.treedetails.meta);
        }
        else {
          this.seoService.setkeywords(this.treedetails.descr);
        }
        if (this.treedetails.meta_descr != undefined && this.treedetails.meta_descr != null && this.treedetails.meta_descr != '') {
          this.seoService.setdescription(this.treedetails.meta_descr);
        }
        else {
          this.seoService.setdescription(this.treedetails.descr);
        }
        if (this.treedetails.title_tag != undefined && this.treedetails.title_tag != null && this.treedetails.title_tag != '') {
          this.seoService.setPageTitle(this.treedetails.title_tag);
        }
        else {
          this.seoService.setPageTitle(this.treedetails.name);
        }
      }
    });
  }
  Getcategoriespath1(param) {
    try {
      if (Common.getWithExpiry('titleheader' + param + '2') != undefined) {
        var titleheader = JSON.parse(Common.getWithExpiry('titleheader' + param + '2'));
      }
    ...
    if (titleheader == null || titleheader == undefined || titleheader.length == 0) {
      this.menuService.Getcategoriespath(param, 2).subscribe((res: any) => {
        this.titleheader = res;
        var geturl = Common.getWithExpiry("cpname");
        for (var i = 0; i < this.titleheader.length; i++) {
          geturl = this.titleheader[i].name + ' - ' + geturl;
        }

        if (this.titleheader == undefined || this.titleheader == null || this.titleheader.length == 0) {
          geturl = 'CATEGORIES - ' + geturl;
          this.seoService.setPageTitle(geturl);
        }
        else if (this.titleheader.length == 1) {
          //geturl = this.titleheader[0].name.toUpperCase() + ' - ' + geturl;
          this.seoService.setPageTitle(geturl);
        }
        // if (this.treedetails.meta != undefined && this.treedetails.meta != null && this.treedetails.meta != '') {
        //   this.seoService.setkeywords(this.treedetails.meta);
        ...
        //   this.seoService.setkeywords(this.treedetails.descr);
        // }
        // if (this.treedetails.seodescr != undefined && this.treedetails.seodescr != null && this.treedetails.seodescr != '') {
        //   this.seoService.setdescription(this.treedetails.seodescr);
        // }        
        // else{
        //   this.seoService.setdescription(this.treedetails.descr);
        // }
        ...
        //   this.seoService.setPageTitle(this.treedetails.name);
        // }        
        Common.setWithExpiry('titleheader' + param + '2', JSON.stringify(this.titleheader))
      });
    }
    else {
  ...
  getAllCategory() {
    ...
    try {
      if (Common.getWithExpiry('categoryList' + getcustomer) != undefined) {
        this.categoryList = JSON.parse(Common.getWithExpiry('categoryList' + getcustomer));
      }
    } catch (ed) { }
    if (this.categoryList == null || this.categoryList == undefined || this.categoryList.length == 0) {
      this.menuService.getMenu(this.warehouse, '', getcustomer).subscribe((res: any) => {
        ...
        Common.setWithExpiry('categoryList' + getcustomer, JSON.stringify(this.categoryList));
        if (this.treeNode == undefined || this.treeNode == null || this.treeNode == '') {
          var getkeywords = '';
          for (let menu of this.categoryList) {
            getkeywords = getkeywords + ', ' + menu.descr;
          }
          this.seoService.setkeywords(getkeywords);
          this.seoService.setdescription(getkeywords);
  ...
  filterMenuByTreeNode(treeNode) {

    if (treeNode != '') {
      this.Getcategoriespath1(treeNode);
    }
    else {
      this.titleheader = [];
    }
    var getcustomer = Common.getWithExpiry("CustID");
    try {
      if (Common.getWithExpiry("getMenu" + this.warehouse + treeNode + getcustomer) != undefined) {
        this.categoryList = JSON.parse(Common.getWithExpiry("getMenu" + this.warehouse + treeNode + getcustomer));
      }
    } catch (ed) { }
    if (this.categoryList != undefined && this.categoryList != null && this.categoryList.length > 0) {
      ...
      this.menuService.getMenu(this.warehouse, treeNode, getcustomer).subscribe((res: any) => {
        this.categoryList = res;
        Common.setWithExpiry("getMenu" + this.warehouse + treeNode + getcustomer, JSON.stringify(this.categoryList));
        if (this.treeNode == undefined || this.treeNode == null || this.treeNode == '') {
          var getkeywords = '';
          for (let menu of this.categoryList) {
            getkeywords = getkeywords + ', ' + menu.descr;
  ...
  getcheckformenuchicl(treeNode) {
    var flag = false;
    for (let menu of this.categoryList) {
      if (menu.parent_node.toLowerCase() == treeNode.toLowerCase()) {
        flag = true;
        break;
      }
    }
  ...
  searcharray1(nameKey, myArray) {
    var flag = false;
    for (var i = 0; i < myArray.length; i++) {
      if (myArray[i].treeNode.toLowerCase() == nameKey.toLowerCase()) {
        flag = true;
      }
    }
    return flag;
